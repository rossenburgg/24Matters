...
export function createRandomUser():User {   
    return {
        ...
        username: faker.internet.userName(),
        link: faker.internet.url(),
        followers: Array(Math.floor(Math.random() * 10)).fill(null).map((_) => createRandomFollower())
    }
}


export function createRandomThreads(): Thread {
    const author = createRandomUser();
    const mentionedUser = createRandomUser();
...
export function generateThreads():Thread[] { 
    return new Array(50).fill(null).map(() => createRandomThreads());
}
